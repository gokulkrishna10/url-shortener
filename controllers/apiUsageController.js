...
exports.updateAPIUsage = function (req, res, mainCallback) {
    ...
    async.waterfall([
            function updateErrorDetails(callback) {
                if (req.isValidationError || (req.body.apiDetails.validationResult !== true)) {  // insert into error table only in case of an internal validation error or internal processing error. And also in the case of apiUsage validation endpoint error
